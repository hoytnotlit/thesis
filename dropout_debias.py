import data, score, self_debias as sdb, context as c
import json
import torch
...
def run_bias_analysis():
    ...
    long_ant = data.get_context_sentences(c.context_long, c.context_long_t_i, c.context_long_a_i, pos=True)
    ...
    sdb.save_scores(get_bert_probs(s_a, model, tokenizer, old_model, old_tokenizer, "s_", save_dist=False), 'dro_short_ant.json')
    sdb.save_scores(get_bert_probs(l, model, tokenizer, old_model, old_tokenizer, "l_"), 'dro_long.json')
    sdb.save_scores(get_bert_probs(l_a, model, tokenizer, old_model, old_tokenizer, "l_", save_dist=False), 'dro_long_ant.json')
